	...
	*filterMsgs = NULL;
	...
	while( fgets( line, MSG_FILTER_MAX_LINE_LEN, filterFile ) ) {
			...
			*lineNum = linesRead + 1;
			...
			// Read in tag.
			i++; if( !isspace( line[ i ] ) ) return cleanUpAndReturn( newMsg, ERROR_READING_FILE ); 
			...
			// Read in msg.
			if( !isspace( line[i] ) ) return cleanUpAndReturn( newMsg, ERROR_READING_FILE ); // Expected white space. Error! 	
